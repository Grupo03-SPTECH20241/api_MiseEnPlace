...
public class MatrizPedidoController {
    ...
    @GetMapping("/{idPedido}")
    public ResponseEntity<Void> criarRelatorio(@PathVariable Integer idPedido) {
        ...
        Pedido pedido = matrizPedidoService.getPedido(idPedido);
        ...
        for (int linha = 1; linha < produtos.size() + 1; linha++) {
            matriz[linha][0] = produtos.get(linha).getNome();
            matriz[linha][1] = produtos.get(linha).getPreco().toString();
            matriz[linha][2] = quantidades.get(linha).toString();
        }
